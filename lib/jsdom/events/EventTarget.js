...
var MultihandleWeakMap = require("../MultihandleWeakMap");
var EventException = require("./EventException").EventException;

var instances = new MultihandleWeakMap();
...
EventTarget.prototype.addEventListener = function (type, callback, capture) {
  ...
  if (props.events[type] === undefined) {
    props.events[type] = {
      capturing: [],
      bubbling: []
    };
  ...
  var handlers = props.events[type][capture ? "capturing" : "bubbling"];
...
module.exports.addAlias = instances.addAlias.bind(instances);
...
function getListeners(target, type, capturing) {
  var props = instances.get(target);

  var listeners;
  if (props.events[type]) {
    listeners = props.events[type][capturing ? "capturing" : "bubbling"];
  } else {
    listeners = [];
  }
